...
def create(request):
	...
	if check[0] == False:
		for error in check[1]:
			messages.add_message(request, messages.INFO, error, extra_tags="error")
			return redirect('/quotes')
	if check[0] == True:
	...
	return redirect('/quotes')
def add(request, id):
	users = current_user(request)
	favorite = Quote.objects.get(id=id)
	users.favorites.add(favorite)
	return redirect('/quotes')
def users(request, id):   
	users =  User.objects.get(id = id)
	context = {
		'user': users,
		'favorites': users.favorites.all()		
	}
	return render(request, 'quotes_app/landing.html', context)
def remove(request, id):
	users = current_user(request)
	favorite = Quote.objects.get(id=id)
	users.favorites.remove(favorite)
	return redirect('/quotes')
